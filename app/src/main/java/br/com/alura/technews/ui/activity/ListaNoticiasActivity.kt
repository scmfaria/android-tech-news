...
class ListaNoticiasActivity : AppCompatActivity() {
    ...
    private fun buscaNoticias() {
        // LifecycleOwner -> referencia da propria activity ou fragment, pois eles so vao se comunicar ou mandar informações
        // quando essa referencia estiver ativa, ou seja, ela nao tiver sido destruida
        // esse "owner" é tudo que tem um ciclo de vida.
        viewModel.buscaTodos().observe(this, Observer {
            it.dado?.let { adapter.atualiza(it) }
            it.erro?.let { mostraErro(MENSAGEM_FALHA_CARREGAR_NOTICIAS) }
        })
    }
